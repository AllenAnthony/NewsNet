...
class RegisterForm extends React.Component{
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                fetch(req).then((response) => {
                    ...
                    if(data.code == 0){
                        //register successfully, login automatically
                        this.setState({
                            show: false,
                        })

                        message.success(data.msg + "，login automatically", 2, () => {
    ...
    handleConfirmBlur = (e) => {//获取密码框中鼠标的焦点
        const value = e.target.value;
        this.setState({ confirmDirty: this.state.confirmDirty || !!value });
    }
    checkPassword = (rule, value, callback) => {//验证两次输入的密码
    ...
    checkConfirm = (rule, value, callback) => {//验证密码是否为空
        const form = this.props.form;
